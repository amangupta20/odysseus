...
async def _call_mcp_tool(
    ...
    if owner:
        args["__mcp_user"] = owner
    if session_id:
        args["__mcp_session_id"] = session_id
        
    result = await mcp.call_tool(qualified, args)
    _persist_mcp_images_to_gallery(qualified, result, owner)
...
def _persist_mcp_images_to_gallery(tool_name: str, result: Dict, owner: Optional[str]) -> None:
    """Intercept MCP ImageContent payloads and persist them to the gallery."""
    ...
    try:
        ...
        for img in result["images"]:
            ...
            server_name = tool_name.split("__")[1] if tool_name.startswith("mcp__") else tool_name
            
            db.add(GalleryImage(
                id=img_id,
                filename=filename,
                prompt=f"Generated by {tool_name}",
                model=server_name,
                size="Unknown",
            ...
            saved_urls.append(f"{_pub_base}/api/generated-image/{filename}")
            
        db.commit()
        
        if saved_urls:
            # Promote to an ImageBubble, delete raw base64 so it's not double-rendered
            result["image_url"] = saved_urls[0]
            result["image_model"] = tool_name.split("__")[1] if tool_name.startswith("mcp__") else tool_name
            del result["images"]
            
    except Exception as e:
        logging.getLogger(__name__).error(f"Failed to persist MCP image to gallery: {e}")
    finally:
        db.close()
...
async def _execute_tool_block_impl(
    ...
    if tool in _MCP_TOOL_MAP:
        ...
        result = await _call_mcp_tool(tool, content, progress_cb=progress_cb, owner=owner, session_id=session_id)
        ...
        if mcp:
            ...
            if parse_error:
                ...
                _persist_mcp_images_to_gallery(tool, result, owner)
        else:
            desc = f"mcp: {tool}"
            result = {"error": "MCP manager not available", "exit_code": 1}
